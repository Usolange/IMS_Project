...
const { sendCustomSms } = require('./notification');
...
router.post('/requestNewLoan', async (req, res) => {
  ...
  try {
    ...
    // === SMS Notification ===
    const smsText = `Dear member, your loan #${loan_id} of amount ${requested_amount} RWF has been approved and disbursed. Please repay by ${dueDate.toLocaleDateString()}. Thank you.`;
    try {
      await sendCustomSms(phone_disbursed_to, smsText);
      console.log(`SMS sent successfully to ${phone_disbursed_to}`);
    } catch (smsErr) {
      console.error('Failed to send SMS notification:', smsErr.message || smsErr);
    }

    res.status(201).json({ message: 'Loan requested, approved, disbursed and SMS sent.', loan_id });
...
router.post('/payLoan', async (req, res) => {
  ...
  try {
    ...
    // Send SMS after repayment
    if (actualPhone) {
      await sendCustomSms(
        actualPhone,
        `Your payment of RWF ${payAmount.toFixed(2)} has been received for Loan #${loan_id}. Thank you.`
      );
    }
    ...
    if (paymentType.includes('interest') && unpaidInterest) {
      ...
      // Send SMS after interest payment
      if (actualPhone) {
        await sendCustomSms(
          actualPhone,
          `Interest payment of RWF ${interestAmount.toFixed(2)} for Loan #${loan_id} has been recorded.`
        );
      }
    }
    ...
    if (!isFullLoanPaid && paymentType === 'loan_only') {
      ...
      // Send SMS about new interest created
      if (actualPhone) {
        await sendCustomSms(
          actualPhone,
          `New interest of RWF ${newInterest.toFixed(2)} added to Loan #${loan_id}, due on ${interestDueDate.toLocaleDateString()}.`
        );
      }
    }
    ...
    if (updateLoan) {
      ...
      // Send SMS for fully repaid loan
      if (actualPhone) {
        await sendCustomSms(
          actualPhone,
          `Congratulations! Your Loan #${loan_id} has been fully repaid.`
        );
      }
    }
...
//     // Step 1: Check for existing unpaid loans
//     let request = pool.request();
//     request.input('memberId', sql.Int, member_id);
//     const existingLoanRes = await request.query(`
//       SELECT loan_id FROM loans
...
//     // Step 2: Get member phone and iki_id
//     request = pool.request();
//     request.input('memberId', sql.Int, member_id);
//     const memberRes = await request.query(`
//       SELECT iki_id, member_phone_number FROM members_info WHERE member_id = @memberId
...
//     // Step 3: Get round interest rate
//     request = pool.request();
//     request.input('ikiId', sql.Int, iki_id);
//     request.input('roundId', sql.Int, round_id);
//     const ruleRes = await request.query(`
...
//     // Step 4: Compute repayment info
//     const approved_amount = requested_amount;
//     const total_repayable = requested_amount + (requested_amount * interestRate / 100);
//     const requestDate = new Date();
//     const dueDate = new Date();
//     dueDate.setMonth(dueDate.getMonth() + 1); // 1-month deadline

//     // Step 5: Insert loan (initially approved, not disbursed yet)
//     request = pool.request();
//     request.input('member_id', sql.Int, member_id);
//     request.input('iki_id', sql.Int, iki_id);
//     request.input('round_id', sql.Int, round_id);
...
//     // Step 6: Get new loan_id
//     const loanIdRes = await pool.request().query('SELECT TOP 1 loan_id FROM loans ORDER BY loan_id DESC');
//     const loan_id = loanIdRes.recordset[0].loan_id;

//     // ✅ Step 7: Initiate cashout (disbursement)
//     const idempotencyKey = `loan_disb_${loan_id}_${Date.now()}`.slice(0, 32);
//     let cashoutRes;
//     try {
//       cashoutRes = await cashout({
//         amount: requested_amount,
//         phone: phone_disbursed_to,
//         idempotencyKey,
//       });
//     } catch (err) {
//       console.error('❌ Cashout failed:', err.message || err);
//       // Optionally rollback loan record
//       await pool.request()
//         .input('loan_id', sql.Int, loan_id)
//         .query('DELETE FROM loans WHERE loan_id = @loan_id');

//       return res.status(500).json({
//         message: 'Loan approved but disbursement failed. Please try again.',
//         error: err.message || 'Disbursement error'
//       });
//     }

//     const disbursedDate = new Date();
//     const cashoutReference = cashoutRes.ref || cashoutRes.reference || cashoutRes.id;

//     // Step 8: Mark loan as disbursed
//     request = pool.request();
//     request.input('loan_id', sql.Int, loan_id);
//     request.input('disbursed_date', sql.DateTime, disbursedDate);
//     request.input('phone_disbursed_to', sql.VarChar(20), phone_disbursed_to);
//     request.input('status', sql.VarChar(20), 'disbursed');
//     request.input('disbursement_ref', sql.VarChar(50), cashoutReference);
//     await request.query(`
//       UPDATE loans
//       SET disbursed_date = @disbursed_date,
//           phone_disbursed_to = @phone_disbursed_to,
...
//     // Step 9: Create initial interest record
//     const interest_amount = requested_amount * (interestRate / 100);
//     const interestDue = new Date(requestDate);
//     interestDue.setMonth(interestDue.getMonth() + 1);
...
//     // ✅ Response
//     res.status(201).json({
//       message: 'Loan requested, approved, and disbursed successfully.',
//       loan_id,
//       disbursed_to: phone_disbursed_to,
//       disbursement_reference: cashoutReference,
//     });

//   } catch (err) {
//     console.error('[Loan API] Loan creation failed:', err.stack || err);
...
//     // Get member phone number if not provided
//     const phoneRequest = pool.request();
//     const memberPhoneRes = await phoneRequest
//       .input('member_id', sql.Int, member_id)
//       .query('SELECT member_phone_number FROM members_info WHERE member_id = @member_id');
//     const actualPhone = phone_used || memberPhoneRes.recordset[0]?.member_phone_number || null;
//     if (!actualPhone) return res.status(400).json({ message: 'Phone number is required for payment.' });

//     // Fetch unpaid interest if any
//     const interestRequest = pool.request();
//     const interestRes = await interestRequest
//       .input('loan_id', sql.Int, loan_id)
//       .query('SELECT TOP 1 * FROM loan_interest WHERE loan_id = @loan_id AND is_paid = 0 ORDER BY due_date ASC');
...
//     // Determine payment amount and logic based on paymentType
//     if (paymentType === 'loan_and_interest') {
//       payAmount = loan.total_repayable - totalPaid;
//       if (payAmount <= 0) return res.status(400).json({ message: 'Loan already fully paid.' });
//       isFullLoanPaid = true;
...
//     } else if (paymentType === 'loan_only') {
//       if (unpaidInterest && unpaidInterest.is_paid === 0) {
//         return res.status(403).json({ message: 'Interest must be fully paid before loan-only payment.' });
//       }
...
//     // --- Initiate payment with PayPack or your payment API ---

//     // Unique idempotency key max length 32
//     const idempotencyKey = `loan_${loan_id}_member_${member_id}_${Date.now()}`.slice(0, 32);
...
//     if (!paymentRef) {
//       return res.status(500).json({ message: 'No payment reference returned from payment gateway.' });
//     }

//     // --- Insert repayment as PENDING (payment_status='pending'), to be confirmed later ---

//     const repaymentRequest = pool.request();
//     const insertResult = await repaymentRequest
//       .input('loan_id', sql.Int, loan_id)
//       .input('member_id', sql.Int, member_id)
//       .input('amount_paid', sql.Decimal(9, 2), payAmount)
//       .input('payment_method', sql.VarChar(20), 'mobile')
...
//       .input('is_full_payment', sql.Bit, isFullLoanPaid ? 1 : 0)
//       .input('created_at', sql.DateTime, now)
//       .input('payment_status', sql.VarChar(20), 'pending') // <-- pending until confirmed
//       .input('timing_status', sql.VarChar(20), loan.due_date && new Date(loan.due_date) < now ? 'late' : 'on_time')
//       .input('payment_reference', sql.VarChar(50), paymentRef) // store payment ref to track confirmation
//       .query(`
//         INSERT INTO loan_repayments
//         (loan_id, member_id, amount_paid, payment_method, phone_used, payment_date,
//          is_full_payment, created_at, payment_status, timing_status, payment_reference)
//         VALUES (@loan_id, @member_id, @amount_paid, @payment_method, @phone_used, @payment_date,
//          @is_full_payment, @created_at, @payment_status, @timing_status, @payment_reference)
//       `);

//     // Respond immediately so client can confirm payment on phone
//     return res.status(202).json({
//       message: 'Payment initiated. Please confirm payment on your phone.',
//       reference: paymentRef,
//       amount: payAmount,
//       phone: actualPhone,
//       loan_id,
//       member_id,
//       is_full_payment: isFullLoanPaid,
//     });
...
router.get('/remainingLoan/:loan_id', async (req, res) => {
  ...
  try {
    ...
    // 1. Fetch the approved principal amount from 'loans'
    const loanResult = await pool.request()
      .input('loan_id', sql.Int, loan_id)
      .query('SELECT approved_amount FROM loans WHERE loan_id = @loan_id');

    if (loanResult.recordset.length === 0) {
      return res.status(404).json({ message: 'Loan not found.' });
    }

    const approvedAmount = parseFloat(loanResult.recordset[0].approved_amount || 0);

    // 2. Total amount repaid from 'loan_repayments'
    const repaidResult = await pool.request()
      .input('loan_id', sql.Int, loan_id)
      .query(`
        SELECT ISNULL(SUM(amount_paid), 0) AS total_repaid
        FROM loan_repayments
        WHERE loan_id = @loan_id
      `);
    const totalRepaid = parseFloat(repaidResult.recordset[0].total_repaid || 0);

    // 3. Total interest already paid from 'loan_interest'
    const interestPaidResult = await pool.request()
      .input('loan_id', sql.Int, loan_id)
      .query(`
        SELECT ISNULL(SUM(interest_amount), 0) AS total_interest_paid
        FROM loan_interest
        WHERE loan_id = @loan_id AND is_paid = 1
      `);
    const totalInterestPaid = parseFloat(interestPaidResult.recordset[0].total_interest_paid || 0);

    // 4. Calculate principal paid = total repaid - total interest paid
    const principalPaid = totalRepaid - totalInterestPaid;

    // 5. Remaining principal = approved - principalPaid
    const remainingPrincipal = Math.max(approvedAmount - principalPaid, 0);

    // 6. Sum of unpaid interest from 'loan_interest'
    const unpaidInterestResult = await pool.request()
      .input('loan_id', sql.Int, loan_id)
      .query(`
        SELECT ISNULL(SUM(interest_amount), 0) AS unpaid_interest
        FROM loan_interest
        WHERE loan_id = @loan_id AND is_paid = 0
      `);
    const unpaidInterest = parseFloat(unpaidInterestResult.recordset[0].unpaid_interest || 0);

    // 7. Total remaining loan = remaining principal + unpaid interest
    ...
    console.error('[GET /remainingLoan] Error:', err);
    return res.status(500).json({ message: 'Server error while fetching remaining loan.' });
  }
});
